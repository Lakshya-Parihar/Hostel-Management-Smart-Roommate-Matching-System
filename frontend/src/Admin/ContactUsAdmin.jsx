...
import { Inbox, Archive, Eye, X, Check, Clock, Mail, User, MessageSquare, ChevronDown, ChevronUp, Send } from 'lucide-react';
    ...
    <AnimatePresence>
      ...
      >
        ...
        >
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-2">Inquiry Details</h2>
          <div className="mb-4">
            <StatusSelector
          ...
          <div className="space-y-4 text-gray-700 max-h-[60vh] overflow-y-auto pr-2">
            ...
            <div className="p-4 mt-4 border border-gray-200 rounded-md bg-gray-50">
              <strong className="mb-2 font-semibold text-black uppercase">Message : </strong><span className="text-base leading-relaxed whitespace-pre-wrap text-gray-800">{submission.message}</span>
            </div>
          </div>
        </motion.div>
      </motion.div>
...
export default function ContactUsAdmin() {
  ...
  const fetchSubmissions = async () => {
    ...
    try {
      const response = await fetch('http://localhost/HMS/backend/Admin/contactus.php');
      if (!response.ok) {
